...
def index():
    if request.method == 'POST':
        ...
        response = current_app.test_client().post('/api/wordcloud/generate', data={'file': (file, 'chat.txt')})
...
def download_wordcloud():
    response = current_app.test_client().post('/api/wordcloud/generate', data={'file': ('dummy content', 'dummy.txt')})

    if response.status_code == 200:
        filename = 'wordcloud.png'
        img_data = response.data

        return send_file(io.BytesIO(img_data), mimetype='image/png', as_attachment=True, download_name=filename)
    else:
        return render_template('index.html', error="Error generating Word Cloud")
